...
var httpTotalRequests = promauto.NewCounter(prometheus.CounterOpts{
...
func NewGatewayServer() *GatewayServer {
	...
	conn, err1 := grpc.NewClient(grpcAddr,
		...
		grpc.WithStatsHandler(otelgrpc.NewClientHandler()), // openTelemetry трассировка grpc клиента
	)
	...
	mux.Handle("/",
		httpMetricWrapper( // оборачиваем HTTP методы, подсчитываем метрики, установка уровня логирования для запроса
			otelhttp.NewHandler(rmux, "grpc-gateway"), // OpenTelemetry трассировка HTTP вызовов
		),
	)

	// Подменяет swagger.json, указанный в файле swagger-initializer.js сгенерированным logistic_package_api.swagger.json
